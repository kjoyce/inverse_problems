...
if alpha_flag == 1
    RegParam_fn = @(a) a^2*sum(sum((Utb.^2)./(dS2dS1.^2+a).^2))+2*sigma^2*sum(sum(dS2dS1.^2./(dS2dS1.^2+a)));
elseif alpha_flag == 2
    RegParam_fn = @(a) (a^2*sum(sum((Utb.^2)./(dS2dS1.^2+a).^2))-n*sigma^2)^2;
end
alpha = fminbnd(RegParam_fn,0,1);
xalpha = V1*((dS2dS1./(dS2dS1.^2+alpha)).*Utb)*V2';
