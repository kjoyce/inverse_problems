...
%% Generate data and build regularization matrix.
  ...
  alpha               = 1e-4; %fminbnd(RegParam_fn,0,1);
...
%%%%%%%%%%%%%% Initial guess zero
  ...
  while( true )
    ...
    if norm(resid) <= nx*ny*sig; break; end; % end if you actually can
    if not(mod(n,20)) % this just updates the plot
      imagesc(x(nx/4+1:3*nx/4,ny/4+1:3*ny/4)), colorbar, colormap(gray), title(sprintf('n = %d, resid = %.3f <= %.2f',n,norm(resid), nx*ny*sig));
      %imagesc(x,[0,max(x_true(:))]), colorbar, colormap(gray), title(sprintf('n = %d, resid = %.3f <= %.2f',n,norm(resid), nx*ny*sig));
      drawnow;
    end;
  end
